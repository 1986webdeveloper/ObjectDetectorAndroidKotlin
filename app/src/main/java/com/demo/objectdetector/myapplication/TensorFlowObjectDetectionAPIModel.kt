...
class TensorFlowObjectDetectionAPIModel private constructor() : Classifier {
    ...
    override fun recognizeImage(bitmap: Bitmap): List<Classifier.Recognition> {
        ...
        for (i in intValues!!.indices) {
            byteValues?.set(i * 3 + 2, (intValues!![i] and 0xFF).toByte())
            byteValues?.set(i * 3 + 1, (intValues!![i] shr 8 and 0xFF).toByte())
            byteValues?.set(i * 3 + 0, (intValues!![i] shr 16 and 0xFF).toByte())
        }

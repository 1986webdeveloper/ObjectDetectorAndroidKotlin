...
import com.demo.objectdetector.myapplication.env.SplitTimer
...
class TensorFlowYoloDetector : Classifier {
        ...
        for (y in 0 until gridHeight) {
            for (x in 0 until gridWidth) {
                for (b in 0 until NUM_BOXES_PER_BLOCK) {
                    ...
                    val w =
                        (Math.exp(output[offset + 2].toDouble()) * ANCHORS[2 * b + 0]).toFloat() * blockSize
                    val h =
                        (Math.exp(output[offset + 3].toDouble()) * ANCHORS[2 * b + 1]).toFloat() * blockSize
    ...
    companion object {
        ...
        // from files.
        private val ANCHORS =
            doubleArrayOf(1.08, 1.19, 3.42, 4.41, 6.63, 11.38, 9.42, 5.11, 16.62, 10.52)
        ...
        fun create(
            ...
            d.outputNames =
                outputName.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()

...
import com.demo.objectdetector.myapplication.env.Size
...
class ObjectTracker protected constructor(
    ...
    class PointChange(
        ...
        internal var pointDelta: Keypoint? = null
    ...
    init {
        ...
        downsampledFrame =
            ByteArray((frameWidth + DOWNSAMPLE_FACTOR - 1) / DOWNSAMPLE_FACTOR * (frameHeight + DOWNSAMPLE_FACTOR - 1) / DOWNSAMPLE_FACTOR)
    }
    ...
    fun nextFrame(
        ...
        timestampedDeltas.add(
            TimestampedDeltas(
                timestamp,
                getKeypointsPacked(DOWNSAMPLE_FACTOR.toFloat())
            )
        )
        ...
        for (keypoint in lastKeypoints!!.pointDeltas) {
            if (keypoint.wasFound) {
                ...
                val screenPoints = floatArrayOf(
                    ...
                    keypoint.keypointB.x,
                    keypoint.keypointB.y
                )
                canvas.drawRect(
